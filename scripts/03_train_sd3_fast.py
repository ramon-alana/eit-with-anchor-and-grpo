...
FLAGS = flags.FLAGS
config_flags.DEFINE_config_file("config", "configs/grpo.py:base", "Training configuration.")
...
def compute_log_prob(transformer, pipeline, sample, j, embeds, pooled_embeds, config):
    if config.train.cfg:
        noise_pred = transformer(hidden_states=torch.cat([sample["latents"][:, j]] * 2), timestep=torch.cat([sample["timesteps"][:, j]] * 2), encoder_hidden_states=embeds, pooled_projections=pooled_embeds, return_dict=False)[0]
        noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
        noise_pred = noise_pred_uncond + config.sample.guidance_scale * (noise_pred_text - noise_pred_uncond)
    else:
        noise_pred = transformer(hidden_states=sample["latents"][:, j], timestep=sample["timesteps"][:, j], encoder_hidden_states=embeds, pooled_projections=pooled_embeds, return_dict=False)[0]

    # compute the log prob of next_latents given latents under the current model
    prev_sample, log_prob, prev_sample_mean, std_dev_t = sde_step_with_logprob(pipeline.scheduler, noise_pred.float(), sample["timesteps"][:, j], sample["latents"][:, j].float(), prev_sample=sample["next_latents"][:, j].float(), noise_level=config.sample.noise_level, sde_type=config.sample.sde_type)

    return prev_sample, log_prob, prev_sample_mean, std_dev_t


def eval(pipeline, test_dataloader, text_encoders, tokenizers, config, accelerator, global_step, reward_fn, executor, autocast, num_train_timesteps, ema, transformer_trainable_parameters, eit_model):
    if config.train.ema:
    ...
    for test_batch in tqdm(test_dataloader, desc="Eval: ", disable=not accelerator.is_local_main_process, position=0):
        prompts, prompt_metadata = test_batch
        prompt_embeds, pooled_prompt_embeds = compute_text_embeddings(prompts, text_encoders, tokenizers, max_sequence_length=128, device=accelerator.device)
        valence_arousal = [(m["valence"], m["arousal"]) for m in prompt_metadata]
        valence_list = [va[0] for va in valence_arousal]
        arousal_list = [va[1] for va in valence_arousal]
        valence = torch.tensor(valence_list, dtype=prompt_embeds.dtype, device=accelerator.device).unsqueeze(-1)
        arousal = torch.tensor(arousal_list, dtype=prompt_embeds.dtype, device=accelerator.device).unsqueeze(-1)
        prompt_embeds = eit_model(inputs_embeds=prompt_embeds, arousal=arousal, valence=valence)[0]
    ...
    last_batch_prompt_ids = tokenizers[0](prompts, padding="max_length", max_length=256, truncation=True, return_tensors="pt").input_ids.to(accelerator.device)
    ...
    if accelerator.is_main_process:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for idx, (prompt, reward) in enumerate(zip(sampled_prompts, sampled_rewards)):
                img_path = os.path.join(tmpdir, f"{idx}.jpg")
                image = Image.open(img_path)
                image = np.array(image)

                # 处理 reward 可能是 Tensor 或 dict 的情况
                if isinstance(reward, dict):
                    caption = f"{prompt:.1000} | " + " | ".join(f"{k}: {v:.2f}" for k, v in reward.items() if v != -10)
                else:  # Tensor 或其他标量
                    caption = f"{prompt:.1000} | reward: {reward:.2f}"

                writer.add_image(f"eval_images/{idx}", image, global_step=global_step, dataformats="HWC")
                writer.add_text(f"eval_prompts/{idx}", caption, global_step=global_step)
...
def main(_):
    ...
    model_config = GPT2Config.from_pretrained(config.eit.config_dir)
    eit_model = EmotionInjectionTransformer(model_config, final_out_type="Linear+LN", sd_feature_dim=config.eit.sd_feature_dim)
    from safetensors.torch import load_model

    load_model(eit_model, config.eit.eit_ckpt)
    logger.info(f"Loaded EIT checkpoint from {config.eit.eit_ckpt}")
    eit_model.requires_grad_(False)
    # ------------ ------------ ------------ ------------ ------------
    ...
    eit_model.to(accelerator.device, dtype=inference_dtype)
    ...
    optimizer = optimizer_cls(transformer_trainable_parameters, lr=config.train.learning_rate, betas=(config.train.adam_beta1, config.train.adam_beta2), weight_decay=config.train.adam_weight_decay, eps=config.train.adam_epsilon)

    # prepare prompt and reward fn
    reward_fn = getattr(flow_grpo.rewards, "multi_score")(accelerator.device, config.reward_fn)
    eval_reward_fn = getattr(flow_grpo.rewards, "multi_score")(accelerator.device, config.reward_fn)

    if config.prompt_fn == "valence_arousal":
        train_dataset = ValenceArousalPromptDataset(csv_path=config.train_txt_path)
        test_dataset = ValenceArousalPromptDataset(csv_path=config.test_txt_path)

        # Create an infinite-loop DataLoader
        train_sampler = DistributedKRepeatSampler(dataset=train_dataset, batch_size=config.sample.train_batch_size, k=config.sample.num_image_per_prompt, num_replicas=accelerator.num_processes, rank=accelerator.process_index, seed=42)

        train_dataloader = DataLoader(train_dataset, batch_sampler=train_sampler, num_workers=1, collate_fn=ValenceArousalPromptDataset.collate_fn)

        test_dataloader = DataLoader(test_dataset, batch_size=config.sample.test_batch_size, collate_fn=ValenceArousalPromptDataset.collate_fn, shuffle=False, num_workers=8)
    ...
    while True:
        ...
        if epoch and epoch % config.eval_freq == 0:
            eval(pipeline, test_dataloader, text_encoders, tokenizers, config, accelerator, global_step, eval_reward_fn, executor, autocast, num_train_timesteps, ema, transformer_trainable_parameters, eit_model)
        ...
        samples = []
        prompts = []
        for i in tqdm(range(config.sample.num_batches_per_epoch), desc=f"Epoch {epoch}: sampling", disable=not accelerator.is_local_main_process, position=0):
            train_sampler.set_epoch(epoch * config.sample.num_batches_per_epoch + i)
            prompts, prompt_metadata = next(train_iter)

            prompt_embeds, pooled_prompt_embeds = compute_text_embeddings(prompts, text_encoders, tokenizers, max_sequence_length=128, device=accelerator.device)
            #  情感注入接口 - 使用 prompt_metadata 中的 valence/arousal
            valence_arousal = [(m["valence"], m["arousal"]) for m in prompt_metadata]
            valence_list = [va[0] for va in valence_arousal]
            arousal_list = [va[1] for va in valence_arousal]
            valence = torch.tensor(valence_list, dtype=prompt_embeds.dtype, device=accelerator.device).unsqueeze(-1)
            ...
            prompt_ids = tokenizers[0](prompts, padding="max_length", max_length=256, truncation=True, return_tensors="pt").input_ids.to(accelerator.device)
        ...
        for sample in tqdm(samples, desc="Waiting for rewards", disable=not accelerator.is_local_main_process, position=0):
            ...
            # print(rewards)
            sample["rewards"] = {key: torch.as_tensor(value, device=accelerator.device).float() for key, value in rewards.items()}
        ...
        if epoch % 10 == 0 and accelerator.is_main_process:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                for idx, (prompt, reward) in enumerate(zip(sampled_prompts, sampled_rewards)):
                    ...
                    writer.add_image(f"train_images/{idx}", image, global_step=global_step, dataformats="HWC")
                    writer.add_text(f"train_prompts/{idx}", caption, global_step=global_step)
        ...
        if accelerator.is_main_process:
            xx = {f"reward_{key}": float(value.mean().item()) for key, value in gathered_rewards.items() if "_strict_accuracy" not in key and "_accuracy" not in key}  # 这里直接转原生 float！
            accelerator.log({"epoch": epoch, **xx}, step=global_step)
        ...
        if config.per_prompt_stat_tracking:
            ...
            if accelerator.is_main_process:
                accelerator.log({"group_size": group_size, "trained_prompt_num": trained_prompt_num, "zero_std_ratio": zero_std_ratio, "reward_std_mean": reward_std_mean}, step=global_step)
            stat_tracker.clear()
        else:
        ...
        samples["advantages"] = advantages.reshape(accelerator.num_processes, -1, advantages.shape[-1])[accelerator.process_index].to(accelerator.device)
        if accelerator.is_local_main_process:
        ...
        if accelerator.is_main_process:
            accelerator.log({"actual_batch_size": mask.sum().item() // config.sample.num_batches_per_epoch}, step=global_step)
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            samples_batched = {k: v.reshape(-1, total_batch_size // config.sample.num_batches_per_epoch, *v.shape[1:]) for k, v in samples.items()}
            ...
            for i, sample in tqdm(list(enumerate(samples_batched)), desc=f"Epoch {epoch}.{inner_epoch}: training", position=0, disable=not accelerator.is_local_main_process):
                ...
                for j in tqdm(train_timesteps, desc="Timestep", position=1, leave=False, disable=not accelerator.is_local_main_process):
                    with accelerator.accumulate(transformer):
                        ...
                        # grpo logic
                        advantages = torch.clamp(sample["advantages"][:, j], -config.train.adv_clip_max, config.train.adv_clip_max)
